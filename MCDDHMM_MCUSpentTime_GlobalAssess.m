...
classdef MCDDHMM_MCUSpentTime_GlobalAssess   
    ...
    methods(Access = public, Static)
        ...
        function [ status ] = AssesSignals(signals, signals_AllValues, currentWorkingScenarioInfo)
            ...
            MCDDHMM_run_cntr_wrt    = signals('MCDDHMM_run_cntr_wrt');
            MCDDHMM_idle_cntr_wrt   = signals('MCDDHMM_idle_cntr_wrt');
            MCDDHMM_wkp_cntr_wrt    = signals('MCDDHMM_wkp_cntr_wrt');
            
            
            % Logic
            if (current_sample_time_val==1)
                if(MCDDHMM_run_cntr_wrt == 0 && MCDDHMM_idle_cntr_wrt == 1 && MCDDHMM_wkp_cntr_wrt == 0)
                    status = AssessmentStatus.PASSED;
                else
                    status = AssessmentStatus.FAILED;
                end
            elseif (current_sample_time_val==2)
                if(MCDDHMM_run_cntr_wrt == 0 && MCDDHMM_idle_cntr_wrt == 2 && MCDDHMM_wkp_cntr_wrt == 1)
                    status = AssessmentStatus.PASSED;
                else
                    status = AssessmentStatus.FAILED;
                end
            elseif (current_sample_time_val==3)
                if(MCDDHMM_run_cntr_wrt == 0 && MCDDHMM_idle_cntr_wrt == 3 && MCDDHMM_wkp_cntr_wrt == 1)
                    status = AssessmentStatus.PASSED;
                else
                    status = AssessmentStatus.FAILED;
                end
            elseif (current_sample_time_val==4)
                if(MCDDHMM_run_cntr_wrt == 0 && MCDDHMM_idle_cntr_wrt == 4 && MCDDHMM_wkp_cntr_wrt == 1)
                    status = AssessmentStatus.PASSED;
                else
                    status = AssessmentStatus.FAILED;
                end
            elseif (current_sample_time_val==5)
                if(MCDDHMM_run_cntr_wrt == 1 && MCDDHMM_idle_cntr_wrt == 4 && MCDDHMM_wkp_cntr_wrt == 1)
                    status = AssessmentStatus.PASSED;
                else
                    status = AssessmentStatus.FAILED;
                end
            elseif (current_sample_time_val==6)
                if(MCDDHMM_run_cntr_wrt == 2 && MCDDHMM_idle_cntr_wrt == 4 && MCDDHMM_wkp_cntr_wrt == 2)
                    status = AssessmentStatus.PASSED;
                else
                    status = AssessmentStatus.FAILED;
                end
            elseif (current_sample_time_val==7)
                if(MCDDHMM_run_cntr_wrt == 3 && MCDDHMM_idle_cntr_wrt == 4 && MCDDHMM_wkp_cntr_wrt == 2)
                    status = AssessmentStatus.PASSED;
                else
                    status = AssessmentStatus.FAILED;
                end
            ...
            elseif (current_sample_time_val==10)
                if(MCDDHMM_run_cntr_wrt == 3 && MCDDHMM_idle_cntr_wrt == 5 && MCDDHMM_wkp_cntr_wrt == 3)
                    status = AssessmentStatus.PASSED;
                else
                    status = AssessmentStatus.FAILED;
                end
         ...
         function [ assessment ] = AssessmentDescription()
			...
			assessment.SignalsToReport 	   = {'MCDDHMM_run_cntr_wrt, MCDDHMM_idle_cntr_wrt, MCDDHMM_wkp_cntr_wrt'}; %for testing
